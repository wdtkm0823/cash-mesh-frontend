...
export default function Home() {
  ...
  const handleExpenseEdit = (transaction: Transaction) => {
    setEditingExpense(transaction);
    setExpenseEditOpen(true);
  };

  const handleExpenseUpdateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    try {
      await apiClient.put(
        `/api/v1/transactions/${editingExpense.id}/`,
        requestData
      );
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-black p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <Dialog open={expenseEditOpen} onOpenChange={setExpenseEditOpen}>
            <DialogContent>
              <form onSubmit={handleExpenseUpdateSubmit}>
                <DialogHeader>
                ...
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      name="date"
                      type="date"
                      defaultValue={editingExpense?.transaction_date}
                      required
                    />
                  </div>
                  ...
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleExpenseEdit(transaction)}
                      >
                        編集
                      </Button>
                      <Button
                        variant="destructive"
                        size="sm"
                        onClick={() => handleExpenseDelete(transaction.id)}
                      >
                        削除
                      </Button>
                    </div>
